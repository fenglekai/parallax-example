...
export default function Destroy() {
  ...
  const sceneInput = useRef<HTMLDivElement | null>(null);
    ...
    scene.current &&
      ...
      setParallax(
        new Parallax(scene.current, {
          hoverOnly: true,
          relativeInput: true,
          inputElement: sceneInput.current,
        })
      );
  }, []);

  return (
    <main className="min-h-screen overflow-hidden flex justify-center items-center">
      <div
        ref={scene}
        id="scene"
        className="scene border"
        data-input-element="#scene-input"
      >
        <div data-depth="1.00">
      ...
      <div ref={sceneInput} id="scene-input" className="scene border">
        <div>
          <Image src={Layout6} alt="layer6"></Image>
        </div>
      </div>
    </main>
  );
